...
app_name = 'data'
urlpatterns = [
    path('boards', views.data.as_view(), name='data'),    
    
    # path('boards', views.boards.as_view(), name='boards'),
    # re_path('^lists/(?P<boardname>.+)$',views.tasks.as_view(),name='lists'),
    # re_path('^tasks/(?P<boardname>.+)/(?P<listname>.+)$',views.cards.as_view(),name='tasks'),
